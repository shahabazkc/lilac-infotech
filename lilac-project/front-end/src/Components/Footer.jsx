import { Apple, Headset, PlaylistAddRounded } from '@material-ui/icons'
import React from 'react'

function Footer() {
    return (
        <footer className="footer-section">
            <div className="p-4">

                <div className="footer-content pt-5 pb-5">
                    <div className="row">
                        <div className="col-xl-2 col-lg-2 mb-50">
                            <div className="footer-widget">
                                <div className="footer-logo">
                                    <p style={{ color: "white" }} href="index.html">  <img src="data:image/png;base64," />
                                        <span style={{ paddingTop: "12px" }}><b> logoIpsum</b></span></p>
                                </div>
                                <div className="footer-text">
                                    <p>Lorem ipsum dolor sit amet, consec tetur adipisicing elit, sed do eiusmod tempor incididuntut consec tetur adipisicing
                                        elit,Lorem ipsum dolor sit amet.</p>
                                </div>
                                <div className="footer-social-icon">
                                    <span>Follow us</span>
                                    <p ><i className="fab fa-facebook-f facebook-bg"></i></p>
                                    <p ><i className="fab fa-twitter twitter-bg"></i></p>
                                    <p ><i className="fab fa-google-plus-g google-bg"></i></p>
                                </div>
                            </div>
                        </div>
                        <div className="col-xl-2 col-lg-2 col-md-2 mb-30">
                            <div className="footer-widget">
                                <div className="footer-widget-heading">
                                    <h3>Useful Links</h3>
                                </div>
                                <ul>
                                    <li><span style={{ color: "white" }}>Products</span></li>
                                    <li><span style={{ color: "white" }}>Classifieds</span></li>
                                    <li><span style={{ color: "white" }}>Contact Us</span></li>
                                    <li><span style={{ color: "white" }}>Login</span></li>
                                    <li><span style={{ color: "white" }}>Signup</span></li>
                                </ul>
                            </div>
                        </div>
                        <div className="col-xl-2 col-lg-2 col-md-2 mb-30">
                            <div className="footer-widget">
                                <div className="footer-widget-heading">
                                    <h3>Customer Area</h3>
                                </div>
                                <ul>
                                    <li><span style={{ color: "white" }}>Products</span></li>
                                    <li><span style={{ color: "white" }}>Classifieds</span></li>
                                    <li><span style={{ color: "white" }}>Contact Us</span></li>
                                    <li><span style={{ color: "white" }}>Login</span></li>
                                    <li><span style={{ color: "white" }}>Signup</span></li>
                                </ul>
                            </div>
                        </div>
                        <div className="col-xl-2 col-lg-2 col-md-2 mb-30">
                            <div className="footer-widget">
                                <div className="footer-widget-heading">
                                    <h3>Vendor Area</h3>
                                </div>
                                <ul>
                                    <li><span style={{ color: "white" }}>Products</span></li>
                                    <li><span style={{ color: "white" }}>Classifieds</span></li>
                                    <li><span style={{ color: "white" }}>Contact Us</span></li>
                                    <li><span style={{ color: "white" }}>Login</span></li>
                                    <li><span style={{ color: "white" }}>Signup</span></li>
                                </ul>
                            </div>
                        </div>
                        <div className="col-xl-2 col-lg-2 col-md-2 mb-50">
                            <div className="footer-widget">
                                <div className="footer-widget-heading">
                                    <h3>Contact</h3>
                                </div>
                                <div className="footer-text mb-25">
                                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vero, inventore.</p>
                                </div>
                                <div className="footer-contact-chat row">
                                    <div className="footer-image col-2">
                                        <Headset />
                                    </div>
                                    <div className="footer-number col-7">
                                        <p>Have any Question? </p>
                                        <p style={{ color: "blue" }}>+123 456 789</p>
                                    </div>
                                    <div className="footer-chat col-3">
                                        <button className="chat-btn-customised-footer">Chat</button>
                                    </div>
                                </div>
                                <div className="footer-stores">
                                    <div className="row">
                                        <div className="col-6"><button className="customised-btn-mobile-app"><Apple />Download on the Apple Store</button></div>
                                        <div className="col-6"><button className="customised-btn-mobile-app"><PlaylistAddRounded />Get it on the Play store</button></div>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </footer>
    )
}

export default Footer
